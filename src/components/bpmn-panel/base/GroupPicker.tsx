...
import { NInput, NButton, NInputGroup, NTag, NModal, NIcon, NDataTable, NEmpty } from 'naive-ui'
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const groups = ref<CamundaLookupItem[]>([])
    ...
    const hasSearchFn = computed(() => !!lookups.searchUserGroups)
    ...
    async function doSearch() {
      if (!lookups.searchUserGroups) return
      loading.value = true
      try {
        const result = await lookups.searchUserGroups(searchKeyword.value)
        groups.value = result
      } finally {
        loading.value = false
      }
    ...
    return () => (
      <div>
            ...
            {!isExpression.value && hasSearchFn.value && (
              <NButton size={props.formSize} type="primary" ghost onClick={openModal}>
                {{
                  icon: () => (
                    <NIcon>
                      <span class="i-ic-baseline-people text-16px" />
                    </NIcon>
                  ),
                }}
              </NButton>
        ...
        <NModal
          ...
          preset="card"
          title={props.label || t('bpmnPanel.fields.groupSelection')}
              ...
              <div style="min-height:400px">
                <NInputGroup class="mb-12px">
                  <NInput
                    ...
                    placeholder={t('bpmnPanel.placeholders.searchGroup')}
                    size={props.formSize}
                    clearable
