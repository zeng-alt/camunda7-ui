...
export default defineComponent({
  ...
  setup(props) {
    ...
    function syncFromModel() {
      ...
      const outEl = extValues.find((v: any) => v.$type === 'camunda:Out' && v.variables === 'all')
      if (outEl) {
        propagateAll.value = true
        local.value = outEl.local === true
      } else {
    ...
    return () => (
      <div>
        <div class="mb-4px text-12px text-#666">{t('bpmnPanel.fields.outMappingPropagation')}</div>
