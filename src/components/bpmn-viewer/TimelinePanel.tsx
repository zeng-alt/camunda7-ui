  ...
  setup(props) {
    ...
    const orderList = computed(() => {
      ...
      return state.executionOrder.map((id, i) => {
        ...
        const status: ExecutionStatus = result && (result.includes('驳回') || result.includes('reject'))
          ? 'rejected'
          : rawStatus
        return {
          id,
                      ...
                      {{
                        default: () => (
                          <div class="flex flex-col gap-2px">
                            {item.result && (
                              <span class="text-12px font-bold" style={{
                                color: item.result.includes('驳回') || item.result.includes('reject')
                                  ? '#dc2626' : '#16a34a',
                              }}>
                                {item.result}
                              </span>
                        ...
                        footer: () => {
                          ...
                          if (item.visitCount > 1) {
                            badges.push(
                              <span class="text-10px text-#2563eb font-medium">↻ {item.visitCount}</span>,
                            )
                          }
                          if (item.rejectCount > 0) {
                            badges.push(
                              <span class="text-10px text-#dc2626 font-bold">✕ {item.rejectCount}</span>,
                            )
                          }
                          if (item.status === 'active') {
                            badges.push(
                              <span class="text-10px text-#2563eb font-medium">● {t('bpmnViewer.tooltip.active')}</span>,
                            )
                          }
                          if (badges.length === 0) return null
                          return (
                            <div class="flex gap-8px mt-4px">{badges}</div>
                          )
                        },
                      }}
                    </NTimelineItem>
